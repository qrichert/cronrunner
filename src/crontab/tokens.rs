...
pub struct CronJob {
    ...
    /// help text for `--fingerprint` and `--tag` modes).
    pub uid: usize,
    ...
    /// user-friendly. (See help text for `--fingerprint` and `--tag`
    /// modes).
    pub fingerprint: u64,
    ...
    /// that the command remains the same. (See help text for
    /// `--fingerprint` and `--tag` modes). This is set by starting the
    /// job's description by `%{...}`, where `...` can be anything but a
    /// closing bracket.
